...
// Calcular ventas del turno actual - DESGLOSE POR MÉTODO DE PAGO
$stmt = $pdo->prepare("
    SELECT 
        ...
        COALESCE(SUM(CASE WHEN metodo_pago IN ('transferencia', 'tarjeta') THEN total ELSE 0 END), 0) as ventas_digital,
        COUNT(CASE WHEN metodo_pago = 'efectivo' THEN 1 END) as tickets_efectivo,
        COUNT(CASE WHEN metodo_pago IN ('transferencia', 'tarjeta') THEN 1 END) as tickets_digital
    FROM Ventas 
    WHERE id_encargado = ? 
    AND fecha_venta >= ?
");
...
$monto_ventas_total = floatval($ventas['total_dinero']);
$monto_ventas_efectivo = floatval($ventas['ventas_efectivo']);
$monto_ventas_digital = floatval($ventas['ventas_digital']);
...
// El dinero esperado en caja SOLO incluye efectivo (inicial + ventas efectivo - retiros)
$monto_esperado = $monto_inicial + $monto_ventas_efectivo - $monto_retiros_total;
...
<body>
    <div class="container">
            ...
            <div class="info-grid">
                ...
                <div class="info-item" style="background: #e8f5e9;">
                    ...
                    <div class="info-value" style="color: #4CAF50;">
                        $<?php echo number_format($monto_ventas_efectivo, 2); ?>
                    </div>
                    <div style="font-size: 12px; color: #666; margin-top: 5px;">
                        <?php echo $ventas['tickets_efectivo']; ?> tickets
                    </div>
                </div>
                <div class="info-item" style="background: #e3f2fd;">
                    <div class="info-label">💳 Ventas Digitales</div>
                    <div class="info-value" style="color: #2196F3;">
                        $<?php echo number_format($monto_ventas_digital, 2); ?>
                    </div>
                    <div style="font-size: 12px; color: #666; margin-top: 5px;">
                        <?php echo $ventas['tickets_digital']; ?> tickets
                    </div>
                </div>
                <div class="info-item">
                    <div class="info-label">💰 Ventas Totales</div>
                    <div class="info-value" style="color: #667eea;">
                        $<?php echo number_format($monto_ventas_total, 2); ?>
                    </div>
                </div>
                <div class="info-item">
        ...
        <div class="calculo-box">
            <h2>🧮 Cálculo del Dinero en Caja (Solo Efectivo)</h2>
            ...
            <div class="linea-calculo positivo">
                <span>➕ Ventas en Efectivo (<?php echo $ventas['tickets_efectivo']; ?> tickets):</span>
                <strong class="valor">+ $<?php echo number_format($monto_ventas_efectivo, 2); ?></strong>
            </div>
            
            <?php if ($monto_ventas_digital > 0): ?>
            <div class="linea-calculo" style="background: #e3f2fd; border-radius: 8px; margin: 10px 0;">
                <span>💳 Ventas Digitales (<?php echo $ventas['tickets_digital']; ?> tickets):</span>
                <strong class="valor" style="color: #2196F3;">$<?php echo number_format($monto_ventas_digital, 2); ?></strong>
            </div>
            <div style="padding: 10px; background: #fff3cd; border-radius: 8px; margin: 10px 0; font-size: 14px; color: #856404;">
                ℹ️ Las ventas digitales NO se cuentan en el efectivo de caja (ya están en cuenta bancaria)
            </div>
            <?php endif; ?>
        ...
        <div class="monto-esperado">
            ...
            <div style="font-size: 14px; color: #155724; margin-top: 10px;">
                (Inicial + Ventas Efectivo - Retiros - Pagos Proveedores)
            </div>
            <?php if ($monto_ventas_digital > 0): ?>
            <div style="font-size: 13px; color: #155724; margin-top: 8px; padding: 10px; background: rgba(255,255,255,0.2); border-radius: 8px;">
                💳 + $<?php echo number_format($monto_ventas_digital, 2); ?> en ventas digitales (en cuenta bancaria)
            </div>
            <?php endif; ?>
        </div>
